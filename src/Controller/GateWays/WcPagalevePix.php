...
class WcPagalevePix extends Abstract_Payment {

	public function __construct() {
		$this->services_api = new Api_Model();

		$this->id                 = 'pagaleve-pix';
		$this->icon               = Core::plugins_url( 'assets/images/icons/pagaleve.png' );
		$this->has_fields         = true;
		$this->method_title       = __( 'Pagaleve' );
		$this->method_description = __( 'Metódo de pagamento PAGALEVE Pix 4x sem juros.' );
	...
	public function payment_fields() {
		...
		printf('<div class="wc-%s-container" style="background:transparent;">
				...
				<img id="wc-pagaleve-background" src="%s" alt="" />
			</div>',
			esc_attr( $this->id ),
			$environment,
			esc_url( 'https://assets.pagaleve.com.br/checkout/split.png' )
		);
	}
}
